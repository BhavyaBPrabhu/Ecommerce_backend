...
public class ProductService {
	...
	public ProductDTO createProduct( ProductDTO productDTO) {
		 boolean exists = productRepository.findByNameAndCategoryId(
		                    productDTO.getName(),
	...
	public ProductDTO updateProduct(Long id, @Valid ProductDTO productDTO) {
		...
		Optional<Product> optional =
		        productRepository.findByNameAndCategoryId(
		                productDTO.getName(),
		                productDTO.getCategoryId()
		        );
	...
	private String generateSku(ProductDTO dto) {
	    ...
	    String namePart = dto.getName().substring(0, Math.min(3, dto.getName().length()))
	                         .toUpperCase();
	    Category category = categoryRepository.findById(dto.getCategoryId()).orElseThrow();
	    String categoryPart = category.getName().substring(0, Math.min(3, category.getName().length()))
	                             .toUpperCase();
