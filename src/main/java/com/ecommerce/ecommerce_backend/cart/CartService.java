...
public class CartService {
	...
	public CartResponseDTO  addToCart(CartItemDTO cartItemDTO) {
		...
		//Fetch product
		Product product = productRepository.findById(cartItemDTO.getProduct_id()).orElseThrow(() -> new ResourceNotFoundException("Product not found"));
;
		...
		double totalPrice = cart.getItems().stream().mapToDouble(x -> x.getProduct().getPrice()*x.getQuantity()).sum();
...
//		String username = authentication.getName();
		...
		//Get cart
		 ...
		 CartItem cartItem = cart.getItems().stream().filter(item -> item.getProduct().getId().equals(id))
				 									.findFirst()
